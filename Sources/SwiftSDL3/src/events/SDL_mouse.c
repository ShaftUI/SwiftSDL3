...
static void SDL_PrivateSendMouseMotion(Uint64 timestamp, SDL_Window *window,
                                       SDL_MouseID mouseID, bool relative,
                                       float x, float y);

static void SDLCALL SDL_MouseDoubleClickTimeChanged(void *userdata,
                                                    const char *name,
                                                    const char *oldValue,
                                                    const char *hint) {
...
static void SDLCALL SDL_MouseDoubleClickRadiusChanged(void *userdata,
                                                      const char *name,
                                                      const char *oldValue,
                                                      const char *hint) {
  SDL_Mouse *mouse = (SDL_Mouse *)userdata;
  ...
  if (hint && *hint) {
    ...
    mouse->double_click_radius =
        32; // 32 pixels seems about right for touch interfaces
  }
}

static void SDLCALL SDL_MouseNormalSpeedScaleChanged(void *userdata,
                                                     const char *name,
                                                     const char *oldValue,
                                                     const char *hint) {
  SDL_Mouse *mouse = (SDL_Mouse *)userdata;
...
static void SDLCALL SDL_MouseRelativeSpeedScaleChanged(void *userdata,
                                                       const char *name,
                                                       const char *oldValue,
                                                       const char *hint) {
  SDL_Mouse *mouse = (SDL_Mouse *)userdata;
...
static void SDLCALL SDL_MouseRelativeModeCenterChanged(void *userdata,
                                                       const char *name,
                                                       const char *oldValue,
                                                       const char *hint) {
  SDL_Mouse *mouse = (SDL_Mouse *)userdata;
...
static void SDLCALL SDL_MouseRelativeSystemScaleChanged(void *userdata,
                                                        const char *name,
                                                        const char *oldValue,
                                                        const char *hint) {
  SDL_Mouse *mouse = (SDL_Mouse *)userdata;
...
static void SDLCALL SDL_MouseWarpEmulationChanged(void *userdata,
                                                  const char *name,
                                                  const char *oldValue,
                                                  const char *hint) {
  SDL_Mouse *mouse = (SDL_Mouse *)userdata;
...
static void SDLCALL SDL_TouchMouseEventsChanged(void *userdata,
                                                const char *name,
                                                const char *oldValue,
                                                const char *hint) {
  SDL_Mouse *mouse = (SDL_Mouse *)userdata;
...
static void SDLCALL SDL_VitaTouchMouseDeviceChanged(void *userdata,
                                                    const char *name,
                                                    const char *oldValue,
                                                    const char *hint) {
...
static void SDLCALL SDL_MouseTouchEventsChanged(void *userdata,
                                                const char *name,
                                                const char *oldValue,
                                                const char *hint) {
  SDL_Mouse *mouse = (SDL_Mouse *)userdata;
  bool default_value;
    ...
    (defined(SDL_PLATFORM_IOS) && !defined(SDL_PLATFORM_TVOS))
  default_value = true;
#else
  default_value = false;
#endif
...
static void SDLCALL SDL_PenMouseEventsChanged(void *userdata, const char *name,
                                              const char *oldValue,
                                              const char *hint) {
  SDL_Mouse *mouse = (SDL_Mouse *)userdata;
...
static void SDLCALL SDL_PenTouchEventsChanged(void *userdata, const char *name,
                                              const char *oldValue,
                                              const char *hint) {
  SDL_Mouse *mouse = (SDL_Mouse *)userdata;
...
static void SDLCALL SDL_MouseAutoCaptureChanged(void *userdata,
                                                const char *name,
                                                const char *oldValue,
                                                const char *hint) {
  SDL_Mouse *mouse = (SDL_Mouse *)userdata;
...
static void SDLCALL SDL_MouseRelativeWarpMotionChanged(void *userdata,
                                                       const char *name,
                                                       const char *oldValue,
                                                       const char *hint) {
  SDL_Mouse *mouse = (SDL_Mouse *)userdata;
...
static void SDLCALL SDL_MouseRelativeCursorVisibleChanged(void *userdata,
                                                          const char *name,
                                                          const char *oldValue,
                                                          const char *hint) {
  SDL_Mouse *mouse = (SDL_Mouse *)userdata;
...
static void SDLCALL SDL_MouseIntegerModeChanged(void *userdata,
                                                const char *name,
                                                const char *oldValue,
                                                const char *hint) {
  SDL_Mouse *mouse = (SDL_Mouse *)userdata;
...
bool SDL_PreInitMouse(void) {
  ...
  SDL_AddHintCallback(SDL_HINT_TOUCH_MOUSE_EVENTS, SDL_TouchMouseEventsChanged,
                      mouse);
  ...
  SDL_AddHintCallback(SDL_HINT_MOUSE_TOUCH_EVENTS, SDL_MouseTouchEventsChanged,
                      mouse);

  SDL_AddHintCallback(SDL_HINT_PEN_MOUSE_EVENTS, SDL_PenMouseEventsChanged,
                      mouse);

  SDL_AddHintCallback(SDL_HINT_PEN_TOUCH_EVENTS, SDL_PenTouchEventsChanged,
                      mouse);

  SDL_AddHintCallback(SDL_HINT_MOUSE_AUTO_CAPTURE, SDL_MouseAutoCaptureChanged,
                      mouse);
  ...
  SDL_AddHintCallback("SDL_MOUSE_INTEGER_MODE", SDL_MouseIntegerModeChanged,
                      mouse);

  mouse->was_touch_mouse_events =
      false; // no touch to mouse movement event pending

  mouse->cursor_visible = true;

  SDL_mouse_names = SDL_CreateHashTable(0, true, SDL_HashID, SDL_KeyMatchID,
                                        SDL_DestroyHashValue, NULL);
...
void SDL_PostInitMouse(void) {
  ...
  /* Create a dummy mouse cursor for video backends that don't support true
   * cursors, so that mouse grab and focus functionality will work.
   */
  if (!mouse->def_cursor) {
...
bool SDL_IsMouse(Uint16 vendor, Uint16 product) {
  // Eventually we'll have a blacklist of devices that enumerate as mice but
  // aren't really
  return true;
}
...
void SDL_AddMouse(SDL_MouseID mouseID, const char *name) {
  ...
  SDL_MouseID *mice = (SDL_MouseID *)SDL_realloc(
      SDL_mice, (SDL_mouse_count + 1) * sizeof(*mice));
  if (!mice) {
    return;
  }
  ...
  SDL_InsertIntoHashTable(SDL_mouse_names, (const void *)(uintptr_t)mouseID,
                          SDL_strdup(name), true);
...
void SDL_RemoveMouse(SDL_MouseID mouseID) {
  ...
  if (mouse_index != SDL_mouse_count - 1) {
    SDL_memmove(&SDL_mice[mouse_index], &SDL_mice[mouse_index + 1],
                (SDL_mouse_count - mouse_index - 1) *
                    sizeof(SDL_mice[mouse_index]));
  }
  --SDL_mouse_count;
  ...
  for (int i = 0; i < mouse->num_sources; ++i) {
    ...
    if (source->mouseID == mouseID) {
      ...
      if (i != mouse->num_sources - 1) {
        SDL_memmove(&mouse->sources[i], &mouse->sources[i + 1],
                    (mouse->num_sources - i - 1) * sizeof(mouse->sources[i]));
      }
      --mouse->num_sources;
      break;
    }
...
bool SDL_HasMouse(void) { return (SDL_mouse_count > 0); }
...
const char *SDL_GetMouseNameForID(SDL_MouseID instance_id) {
  ...
  if (!SDL_FindInHashTable(SDL_mouse_names,
                           (const void *)(uintptr_t)instance_id,
                           (const void **)&name)) {
...
SDL_Mouse *SDL_GetMouse(void) { return &SDL_mouse; }

static SDL_MouseButtonFlags SDL_GetMouseButtonState(SDL_Mouse *mouse,
                                                    SDL_MouseID mouseID,
                                                    bool include_touch) {
  int i;
...
bool SDL_MousePositionInWindow(SDL_Window *window, float x, float y) {
  ...
  if (window && !(window->flags & SDL_WINDOW_MOUSE_CAPTURE)) {
    if (x < 0.0f || y < 0.0f || x >= (float)window->w ||
        y >= (float)window->h) {
      return false;
    }
  }
...
static bool SDL_UpdateMouseFocus(SDL_Window *window, float x, float y,
                                 Uint32 buttonstate, bool send_mouse_motion) {
  SDL_Mouse *mouse = SDL_GetMouse();
...
void SDL_SendMouseMotion(Uint64 timestamp, SDL_Window *window,
                         SDL_MouseID mouseID, bool relative, float x, float y) {
  if (window && !relative) {
    SDL_Mouse *mouse = SDL_GetMouse();
    if (!SDL_UpdateMouseFocus(
            window, x, y, SDL_GetMouseButtonState(mouse, mouseID, true),
            (mouseID != SDL_TOUCH_MOUSEID && mouseID != SDL_PEN_MOUSEID))) {
      return;
    }
  }
...
static void ConstrainMousePosition(SDL_Mouse *mouse, SDL_Window *window,
                                   float *x, float *y) {
...
static void SDL_PrivateSendMouseMotion(Uint64 timestamp, SDL_Window *window,
  ...
  float xrel = 0.0f;
  float yrel = 0.0f;
  bool window_is_relative =
      mouse->focus && (mouse->focus->flags & SDL_WINDOW_MOUSE_RELATIVE_MODE);

  // SDL_HINT_MOUSE_TOUCH_EVENTS: controlling whether mouse events should
  // generate synthetic touch events
  if (mouse->mouse_touch_events) {
    if (mouseID != SDL_TOUCH_MOUSEID && mouseID != SDL_PEN_MOUSEID &&
        !relative && track_mouse_down) {
      if (window) {
        float normalized_x = x / (float)window->w;
        float normalized_y = y / (float)window->h;
        SDL_SendTouchMotion(timestamp, SDL_MOUSE_TOUCHID, SDL_BUTTON_LEFT,
                            window, normalized_x, normalized_y, 1.0f);
      }
    }
  }

  // SDL_HINT_TOUCH_MOUSE_EVENTS: if not set, discard synthetic mouse events
  // coming from platform layer
  ...
  if (relative) {
      ...
      if (mouse->InputTransform) {
        ...
        if (mouse->enable_relative_system_scale) {
          if (mouse->ApplySystemScale) {
            mouse->ApplySystemScale(mouse->system_scale_data, timestamp, window,
                                    mouseID, &x, &y);
          }
        }
    ...
    if (mouse->integer_mode_flags & 1) {
      // Accumulate the fractional relative motion and only process the integer
      // portion
      mouse->integer_mode_residual_motion_x =
          SDL_modff(mouse->integer_mode_residual_motion_x + x, &x);
      mouse->integer_mode_residual_motion_y =
          SDL_modff(mouse->integer_mode_residual_motion_y + y, &y);
    }
    xrel = x;
    yrel = y;
    x = (mouse->last_x + xrel);
  ...
  if (mouse->has_position && xrel == 0.0f &&
      yrel == 0.0f) { // Drop events that don't change state
#ifdef DEBUG_MOUSE
  ...
  if (mouseID == SDL_TOUCH_MOUSEID &&
      !SDL_GetMouseButtonState(mouse, mouseID, true)) {
    xrel = 0.0f;
    yrel = 0.0f;
  }
  ...
  if (mouse->cursor_visible && !mouse->relative_mode && mouse->MoveCursor &&
      mouse->cur_cursor) {
  ...
  if (SDL_EventEnabled(SDL_EVENT_MOUSE_MOTION)) {
    ...
    // Set us pending (or clear during a normal mouse movement event) as having
    // triggered
...
static SDL_MouseInputSource *GetMouseInputSource(SDL_Mouse *mouse,
                                                 SDL_MouseID mouseID, bool down,
                                                 Uint8 button) {
  ...
  if (!down && (!match || !(match->buttonstate & SDL_BUTTON_MASK(button)))) {
    /* This might be a button release from a transition between mouse messages
     * and raw input. See if there's another mouse source that already has that
     * button down and use that.
     */
    for (i = 0; i < mouse->num_sources; ++i) {
      source = &mouse->sources[i];
  ...
  sources = (SDL_MouseInputSource *)SDL_realloc(
      mouse->sources, (mouse->num_sources + 1) * sizeof(*mouse->sources));
  if (sources) {
...
static SDL_MouseClickState *GetMouseClickState(SDL_MouseInputSource *source,
                                               Uint8 button) {
  if (button >= source->num_clickstates) {
    int i, count = button + 1;
    SDL_MouseClickState *clickstate = (SDL_MouseClickState *)SDL_realloc(
        source->clickstate, count * sizeof(*source->clickstate));
    if (!clickstate) {
      return NULL;
    }
...
static void SDL_PrivateSendMouseButton(Uint64 timestamp, SDL_Window *window,
                                       SDL_MouseID mouseID, Uint8 button,
  ...
  if (mouse->mouse_touch_events) {
    if (mouseID != SDL_TOUCH_MOUSEID && mouseID != SDL_PEN_MOUSEID &&
      ...
      if (window) {
        ...
        SDL_SendTouch(timestamp, SDL_MOUSE_TOUCHID, SDL_BUTTON_LEFT, window,
                      type, normalized_x, normalized_y, 1.0f);
      }
    }
  }
    ...
    if (clickstate) {
      if (down) {
        ...
        if (now >= (clickstate->last_timestamp + mouse->double_click_time) ||
            SDL_fabs(mouse->click_motion_x - clickstate->click_motion_x) >
                mouse->double_click_radius ||
            SDL_fabs(mouse->click_motion_y - clickstate->click_motion_y) >
                mouse->double_click_radius) {
...
void SDL_SendMouseButtonClicks(Uint64 timestamp, SDL_Window *window,
                               SDL_MouseID mouseID, Uint8 button, bool down,
                               int clicks) {
  clicks = SDL_max(clicks, 0);
  SDL_PrivateSendMouseButton(timestamp, window, mouseID, button, down, clicks);
}

void SDL_SendMouseButton(Uint64 timestamp, SDL_Window *window,
                         SDL_MouseID mouseID, Uint8 button, bool down) {
  SDL_PrivateSendMouseButton(timestamp, window, mouseID, button, down, -1);
}

void SDL_SendMouseWheel(Uint64 timestamp, SDL_Window *window,
  ...
  if (SDL_EventEnabled(SDL_EVENT_MOUSE_WHEEL)) {
    ...
    mouse->residual_scroll_x =
        SDL_modff(mouse->residual_scroll_x + x, &integer_x);
    event.wheel.integer_x = (Sint32)integer_x;

    mouse->residual_scroll_y =
        SDL_modff(mouse->residual_scroll_y + y, &integer_y);
...
void SDL_QuitMouse(void) {
  ...
  SDL_RemoveHintCallback(SDL_HINT_PEN_MOUSE_EVENTS, SDL_PenMouseEventsChanged,
                         mouse);

  SDL_RemoveHintCallback(SDL_HINT_PEN_TOUCH_EVENTS, SDL_PenTouchEventsChanged,
                         mouse);
  ...
  SDL_RemoveHintCallback("SDL_MOUSE_INTEGER_MODE", SDL_MouseIntegerModeChanged,
                         mouse);
...
bool SDL_SetRelativeMouseTransform(SDL_MouseMotionTransformCallback transform,
                                   void *userdata) {
  SDL_Mouse *mouse = SDL_GetMouse();
  if (mouse->relative_mode) {
    return SDL_SetError(
...
void SDL_PerformWarpMouseInWindow(SDL_Window *window, float x, float y,
                                  bool ignore_relative_mode) {
  SDL_Mouse *mouse = SDL_GetMouse();
  ...
  if (mouse->relative_mode && !ignore_relative_mode) {
     ...
     * but doesn't generate motion events, unless
     * SDL_HINT_MOUSE_RELATIVE_WARP_MOTION is set.
     */
    if (!mouse->relative_mode_warp_motion) {
      mouse->x = x;
      mouse->y = y;
...
static void SDL_MaybeEnableWarpEmulation(SDL_Window *window, float x, float y) {
  ...
  if (!mouse->warp_emulation_prohibited && mouse->warp_emulation_hint &&
      !mouse->cursor_visible && !mouse->warp_emulation_active) {
    if (!window) {
      window = mouse->focus;
    }
    ...
    if (window) {
      ...
      if (x >= SDL_floorf(cx) && x <= SDL_ceilf(cx) && y >= SDL_floorf(cy) &&
          y <= SDL_ceilf(cy)) {

        // Require two consecutive warps to the center within a certain timespan
        // to enter warp emulation mode.
        const Uint64 now = SDL_GetTicksNS();
        if (now - mouse->last_center_warp_time_ns <
            WARP_EMULATION_THRESHOLD_NS) {
...
bool SDL_SetRelativeMouseMode(bool enabled) {
  ...
  // Flush pending mouse motion - ideally we would pump events, but that's not
  // always safe
...
void SDL_UpdateRelativeMouseMode(void) {
  ...
  bool relative_mode =
      (focus && (focus->flags & SDL_WINDOW_MOUSE_RELATIVE_MODE));
  ...
  if (!force_release) {
    if (SDL_GetMessageBoxCount() == 0 &&
        (mouse->capture_desired ||
         (mouse->auto_capture &&
          SDL_GetMouseButtonState(mouse, SDL_GLOBAL_MOUSE_ID, false) != 0))) {
      if (!mouse->relative_mode) {
...
SDL_Cursor *SDL_CreateCursor(const Uint8 *data, const Uint8 *mask, int w, int h,
                             int hot_x, int hot_y) {
  SDL_Surface *surface;
  SDL_Cursor *cursor;
  int x, y;
...
static SDL_CursorAnimation *
SDL_CreateCursorAnimation(SDL_CursorFrameInfo *frames, int frame_count,
                          int hot_x, int hot_y) {
  SDL_CursorAnimation *animation =
      (SDL_CursorAnimation *)SDL_calloc(1, sizeof(*animation));
  if (!animation) {
    return NULL;
  }

  animation->frames =
      (SDL_Cursor **)SDL_calloc(frame_count, sizeof(*animation->frames));
  animation->durations =
      (Uint32 *)SDL_calloc(frame_count, sizeof(*animation->durations));
  ...
  for (int i = 0; i < frame_count; ++i) {
    animation->frames[i] =
        SDL_CreateColorCursor(frames[i].surface, hot_x, hot_y);
...
void SDL_UpdateCursorAnimation(void) {
  ...
  animation->current_frame =
      (animation->current_frame + 1) % animation->num_frames;
...
SDL_Cursor *SDL_CreateAnimatedCursor(SDL_CursorFrameInfo *frames,
  ...
  hot_x = (int)SDL_GetNumberProperty(props, SDL_PROP_SURFACE_HOTSPOT_X_NUMBER,
                                     hot_x);
  hot_y = (int)SDL_GetNumberProperty(props, SDL_PROP_SURFACE_HOTSPOT_Y_NUMBER,
                                     hot_y);

  // Sanity check the hot spot
  CHECK_PARAM((hot_x < 0) || (hot_y < 0) || (hot_x >= frames[0].surface->w) ||
              (hot_y >= frames[0].surface->h)) {
    SDL_SetError("Cursor hot spot doesn't lie within cursor");
    return NULL;
  }

  bool isstack;
  SDL_CursorFrameInfo *temp_frames =
      SDL_small_alloc(SDL_CursorFrameInfo, frame_count, &isstack);
  if (!temp_frames) {
    return NULL;
  }
  ...
  for (int i = 0; i < frame_count; ++i) {
    ...
    if (frames[i].surface->format == SDL_PIXELFORMAT_ARGB8888) {
      ...
      SDL_Surface *temp =
          SDL_ConvertSurface(frames[i].surface, SDL_PIXELFORMAT_ARGB8888);
      if (!temp) {
        goto cleanup;
      }
  ...
  if (mouse->CreateAnimatedCursor) {
    cursor =
        mouse->CreateAnimatedCursor(temp_frames, frame_count, hot_x, hot_y);
  } else {
    SDL_CursorAnimation *animation =
        SDL_CreateCursorAnimation(temp_frames, frame_count, hot_x, hot_y);
    if (!animation) {
      goto cleanup;
    }
...
SDL_Cursor *SDL_CreateColorCursor(SDL_Surface *surface, int hot_x, int hot_y) {
  ...
  // Sanity check the hot spot
  CHECK_PARAM((hot_x < 0) || (hot_y < 0) || (hot_x >= surface->w) ||
              (hot_y >= surface->h)) {
    SDL_SetError("Cursor hot spot doesn't lie within cursor");
    return NULL;
  }
...
void SDL_RedrawCursor(void) {
  ...
  if (mouse->focus &&
      (!mouse->cursor_visible ||
       (mouse->relative_mode && mouse->relative_mode_hide_cursor))) {
    cursor = NULL;
  }
